...
*/
int sindex(int n, int m, int cterms)        /* cterms is costerms(order) */
{
#if NOABRT == OFF
  if(m > n) {
...
*/
double iPwr(int e)    /* e: exponent, computes i^e */
{
  if(e == 0) return(1.0);
  if(e % 2 != 0) {
...
/*c2.0 maxsngs now means max # of singularities or fieldpoints in a cube.*/
{
  ...
  CALLOC(facFrA, 2*(unsigned) order+1, double*, ON, AMSC);
  for(x = 0; x < 2*order+1; x++) {
    CALLOC(facFrA[x], 2*(unsigned) order+1, double, ON, AMSC);
  }
  /* generate table of factorial fraction evaluations (for M2L and L2L) */
  evalFacFra(facFrA, order);
  CALLOC(sinmkB, 2*(unsigned) order+1, double, ON, AMSC); /* sin[(m+-k)beta] */
  CALLOC(cosmkB, 2*(unsigned) order+1, double, ON, AMSC); /* cos[(m+-k)beta] */
...
void evalLegendre(double cosA, double *vector, int order)
{
  ...
  double sinMA=0.;      /* becomes sin^m(alpha) in higher order P's */
  double fact;          /* factorial factor */

  /* do evaluations of first four functions separately w/o recursions */
  vector[iindex(0, 0)] = 1.0;    /* P0^0 */
  if(order > 0) {
    vector[iindex(1, 0)] = cosA; /* P1^0 */
    vector[iindex(1, 1)] = sinMA = -sqrt(1-cosA*cosA); /* P1^1 = -sin(alpha) */
  }
  if(order > 1) vector[iindex(2, 1)] = 3*sinMA*cosA; /* P2^1 = -3sin()cos() */
  ...
  for(m = 0; m < order+1; m++) {
    if(m != 0 && m != 1) {  /* set up first two evaluations in row */
      ...
      /* use recursion on m */
      if(vector[iindex(1, 1)] == 0.0) {
    vector[iindex(m, m)] = 0.0;
    if(m != order) vector[iindex(m+1, m)] = 0.0; /* if not last row */
      }
      else {
    cosA = vector[iindex(1,0)]/vector[iindex(1,1)]; /* cosA= -cot(theta) */
    sinMA *= vector[iindex(1,1)]; /*(-sin(alpha))^(m-1)->(-sin(alpha))^m*/
    vector[iindex(m, m)] = fact * sinMA;
    if(m != order) {        /* do if not on last row */
      vector[iindex(m+1, m)] = vector[iindex(1, 0)]*(2*m+1)
          *vector[iindex(m, m)];
    }
      }
    }
    for(x = 2; x < order-m+1; x++) { /* generate row of evals recursively */
      vector[iindex(x+m, m)] =
      ((2*(x+m)-1)*vector[iindex(1, 0)]*vector[iindex(x+m-1, m)]
       - (x + 2*m - 1)*vector[iindex(x+m-2, m)])/x;
    }
  }
}
    ...
    for(m = 0, mBeta = 0.0; m <= n; m++, mBeta += beta) {
      ...
      if(k == 0) {      /* figure terms for Nj^0, ie k = 0 */
        if(m <= j-n) {  /* if O moments are nonzero */
          temp1 = fact(j)*rhoPwr*iPwr(2*m)*tleg[iindex(n, m)];
          temp1 /= (fact(j-n+m)*fact(n+m));
          mat[iindex(j, k)][iindex(j-n, m)] += temp1*cos(mBeta);
          if(m != 0)        /* if sin term is non-zero */
          mat[iindex(j, k)][sindex(j-n, m, cterms)] += temp1*sin(mBeta);
        }
      }
      else {        /* figure terms for Nj^k, k != 0 */
        temp1 = fact(j+k)*rhoPwr*tleg[iindex(n, m)]/fact(n+m);
        ...
        mat[iindex(j, k)][iindex(j-n, m-k)] += temp1*cos(mBeta);
        mat[iindex(j, k)][sindex(j-n, m-k, cterms)] += temp1*sin(mBeta);
          }
          else if(k-m == 0) {   /* double to compensate for 2Re sub. */
        mat[iindex(j, k)][iindex(j-n, k-m)] += 2*temp1*cos(mBeta);
        /* sin term is always zero */
          }
          else if(k-m > 0 && k-m <= j-n) {
        mat[iindex(j, k)][iindex(j-n, k-m)] += temp1*cos(mBeta);
        mat[iindex(j, k)][sindex(j-n, k-m, cterms)] -= temp1*sin(mBeta);
          }
          if(k+m <= j-n) {
        mat[iindex(j, k)][iindex(j-n, k+m)] += temp2*cos(mBeta);
        mat[iindex(j, k)][sindex(j-n, k+m, cterms)] += temp2*sin(mBeta);
          }
        }           /* do if m = 0 and O moments not zero */
        else if(k <= j-n) mat[iindex(j, k)][iindex(j-n, k)] += temp2;

        /* write the sin(kPhi) coeff, dblbar(N)j^k, if it is non-zero */
        if(m != 0) {
          if(k-m < 0 && abs(k-m) <= j-n) {  /* use conjugates here */
            mat[sindex(j, k, cterms)][iindex(j-n, m-k)] += temp1*sin(mBeta);
            mat[sindex(j, k, cterms)][sindex(j-n, m-k, cterms)]
            -= temp1*cos(mBeta);
          }
          else if(k-m == 0) {/* double to compensate for 2Re sub */
            mat[sindex(j, k, cterms)][iindex(j-n, k-m)]
            += 2*temp1*sin(mBeta);
        /* sine term is always zero */
          }
          else if(k-m > 0 && k-m <= j-n) {
            mat[sindex(j, k, cterms)][iindex(j-n, k-m)] += temp1*sin(mBeta);
            mat[sindex(j, k, cterms)][sindex(j-n, k-m, cterms)]
            += temp1*cos(mBeta);
          }
          if(k+m <= j-n) {
            mat[sindex(j, k, cterms)][iindex(j-n, k+m)] -= temp2*sin(mBeta);
            mat[sindex(j, k, cterms)][sindex(j-n, k+m, cterms)]
            += temp2*cos(mBeta);
          }
...
/* multipole expansion origin */
{
  ...
  for(n = 1; n <= order; n++) {
    for(m = 1; m <= n; m++) {
      for(i = 0; i < numfpts; i++) mat[i][iindex(n, m)] /= factFac[n][m];
    }
  }
  ...
  for(i = 0; i < numfpts; i++) { /* loop on rows of matrix */
    ...
    mat[i][sindex(n, m, cterms)] = mat[i][iindex(n, m)];
      }
    }
  }
  ...
  for(i = 0; i < numfpts; i++)
    if(fpts[i]->deriv == 1) {
      for(n = 0; n < order; n++) {
        /* m=0 */
        factorial = (n+2)*(n+1);
        matP1 = factorial*mat[i][iindex(n+1, 1)];
        matP2 = factorial*mat[i][sindex(n+1, 1, cterms)];
        matC1 = (n+1)*mat[i][iindex(n+1, 0)];
        tmp  = fpts[i]->nrm[0]*matP1;
        tmp += fpts[i]->nrm[1]*matP2;
        tmp -= fpts[i]->nrm[2]*matC1;
        mat[i][iindex(n, 0)] = tmp;
        for(m = 1; m <= n; m++) {
          matM1 = mat[i][iindex(n+1, m-1)];
          if (m == 1)
            matM2 = 0.0;
          else
      ...
      mat[i][iindex(order, 0)] = 0.0;
      for(m = 1; m <= order ; m++) {
        mat[i][iindex(order, m)] = 0.0;
