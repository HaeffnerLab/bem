...
/* array is 2*order+1 x 2*order+1 */
/* array[num][den] = num!/den! */
{
  int d, i;
  for(i = 0; i <= 2*order; i++) {
...
/* x, y, z, xp, yp, zp: multipole and local cube centers */
/* **mat: Place to store matrix, typically null. */
{
  int i, j, k, n, m;
  ...
  for(j = 0, rhoJ = 1.0; j <= order; rhoJ *= rho, j++) {
    ...
    for(m = 0; m <= n; m++) {  /* loop on On^m's, multipole moments */
      ...
      if(k == 0) {         /* use abbreviated formulae in this case */
        ...
        if(m != 0) {
          temp1 = tleg[iindex(j+n, m)]*facFrA[j+n-m][n+m];
          mat[iindex(j, 0)][iindex(n, m)] += temp1*cosB(m)*rhoFac;
          mat[iindex(j, 0)][sindex(n, m, ct)] += temp1*sinB(m)*rhoFac;
        }
        else mat[iindex(j, 0)][iindex(n, 0)]
        += tleg[iindex(j+n, 0)]*facFrA[j+n][n]*rhoFac;
      }
      else {
        temp1 = tleg[iindex(j+n, abs(m-k))]
        *facFrA[j+n-abs(m-k)][n+m]*iPwr(abs(k-m)-k-m);
        temp2 = tleg[iindex(j+n, m+k)]*facFrA[j+n-m-k][n+m];
        temp3 = tleg[iindex(j+n, k)]*facFrA[j+n-k][n]*2;

        /* generate bar(N)j^k entry */
        if(m != 0) {
          mat[iindex(j, k)][iindex(n, m)]
          += (temp1*cosB(m-k)+temp2*cosB(m+k))*rhoFac;
          mat[iindex(j, k)][sindex(n, m, ct)]
          += (temp1*sinB(m-k)+temp2*sinB(m+k))*rhoFac;
        }
        else mat[iindex(j, k)][iindex(n, 0)] += temp3*cosB(k)*rhoFac;

        /* generate dblbar(N)j^k entry */
        if(m != 0) {
          mat[sindex(j, k, ct)][iindex(n, m)]
          += (-temp1*sinB(m-k)+temp2*sinB(m+k))*rhoFac;
          mat[sindex(j, k, ct)][sindex(n, m, ct)]
          += (temp1*cosB(m-k)-temp2*cosB(m+k))*rhoFac;
        }
        else mat[sindex(j, k, ct)][iindex(n, 0)] += temp3*sinB(k)*rhoFac;
      }
    }
      }
    }
...
/* x, y, z, xc, yc, zc: Parent and child cube centers */
{
  int i, j, k, n, m;
  int terms = multerms(order);  /* the number of non-zero moments */
  int ct = costerms(order);     /* the number of non-zero cos (bar) moments */
  ...
  for(j = 0, rhoJ = 1.0; j <= order; rhoJ *= (-rho), j++) {
    ...
    for(m = 0; m <= n; m++) { /* loop on On^m's, old local moments */
      ...
      if(k == 0 && n-j >= m) {  /* use abbreviated formulae in this case */
        ...
        /* generate only bar(N)j^0 entry (dblbar(N)j^0 = 0 always) */
        if(m != 0) {
          temp1 = tleg[iindex(n-j, m)]*facFrA[0][n-j+m]*rhoFac;
          mat[iindex(j, 0)][iindex(n, m)] += temp1*cosB(m);
          mat[iindex(j, 0)][sindex(n, m, ct)] += temp1*sinB(m);
        }
        else mat[iindex(j, 0)][iindex(n, 0)] += tleg[iindex(n-j, 0)]
            *facFrA[0][n-j]*rhoFac;
      }
      else {
        if(n-j >= abs(m-k)) temp1 = tleg[iindex(n-j, abs(m-k))]
        *facFrA[0][n-j+abs(m-k)]*iPwr(m-k-abs(m-k))*rhoFac;
        if(n-j >= m+k) temp2 = tleg[iindex(n-j, m+k)]
        *facFrA[0][n-j+m+k]*iPwr(2*k)*rhoFac;
        if(n-j >= k) temp3 = 2*tleg[iindex(n-j, k)]
        *facFrA[0][n-j+k]*iPwr(2*k)*rhoFac;

        /* generate bar(N)j^k entry */
        if(m != 0) {
          if(n-j >= abs(m-k)) {
        mat[iindex(j, k)][iindex(n, m)] += temp1*cosB(m-k);
        mat[iindex(j, k)][sindex(n, m, ct)] += temp1*sinB(m-k);
          }
          if(n-j >= m+k) {
        mat[iindex(j, k)][iindex(n, m)] += temp2*cosB(m+k);
        mat[iindex(j, k)][sindex(n, m, ct)] += temp2*sinB(m+k);
          }
        }
        else if(n-j >= k) mat[iindex(j, k)][iindex(n, 0)] += temp3*cosB(k);

        /* generate dblbar(N)j^k entry */
        if(m != 0) {
          if(n-j >= abs(m-k)) {
        mat[sindex(j, k, ct)][iindex(n, m)] += (-temp1*sinB(m-k));
        mat[sindex(j, k, ct)][sindex(n, m, ct)] += temp1*cosB(m-k);
          }
          if(n-j >= m+k) {
        mat[sindex(j, k, ct)][iindex(n, m)] += (-temp2*sinB(m+k));
        mat[sindex(j, k, ct)][sindex(n, m, ct)] += temp2*cosB(m+k);
          }
        }
        else if(n-j >= k) 
        mat[sindex(j, k, ct)][iindex(n, 0)] += temp3*sinB(k);
      }
    }
      }
    }
...
/* x, y, z: This is the center for the local expansion. */
/* **sngs: Array of pointers to the snglrty structures. */
/* umsngs, order: Number of snglrtys in this cube, order of expansions. */
/* mat contains translation, usually NULL */
{
  int i, j, k;
...
double **mulLocal2P(double x, double y, double z, fieldpt **fpts, int numfpts, int order)
{
  ...
  for(n = 0; n <= order; n++) {
    for(m = 0; m <= n; m++) {
      for(i = 0; i < numfpts; i++) mat[i][iindex(n, m)] /= fact(n+m);
    }
  }
...
/*cdel nr_mulLocal_calls++;*/
  ...
  for(i = 0; i < numfpts; i++) {
    if(fpts[i]->deriv == 1) {
      for(n = order; n >= 2; n--) {
        /* m=0 */
        matP1 = mat[i][iindex(n-1, 1)];
        matP2 = mat[i][sindex(n-1, 1, cterms)];
        matC1 = mat[i][iindex(n-1, 0)];
      ...
      /* n = 1 */
      matC1 = mat[i][iindex(0, 0)];
      mat[i][iindex(1, 0)] = fpts[i]->nrm[2]*matC1;
      mat[i][iindex(1, 1)] = -0.5*fpts[i]->nrm[0]*matC1;
      mat[i][sindex(1, 1, cterms)] = -0.5*fpts[i]->nrm[1]*matC1;
      /* n = 0 */
      mat[i][iindex(0,0)] = 0.0;
    }
  }    
