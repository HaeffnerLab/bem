...
#if (PRECOND == OL) | (PRECOND == SP)
static void getqnbrs(ssystem *sys);
#endif
#if PRECOND == OL
static void getpnbrs(ssystem *sys);
#endif
...
static int placeq(int flag, ssystem *sys, snglrty *snglrtys, int lhsSize, int rhsSize, fieldpt *fieldpts)
{
  ...
  if(flag == ON) {      /* set depth of partitions automatically */
    ...
    for(k=0; k < side; k++) {
      ...
      /* Count the number of singularities per cube and allocate if needed */
...
/*cftk seems to me upnumvects is somewhat redundant and could be purged.*/
    ...
    for(nextf = fieldpts; nextf != NULL; nextf = nextf->next) {
      xindex = (int) ((nextf->x - minx) / length);
      yindex = (int) ((nextf->y - miny) / length);
      zindex = (int) ((nextf->z - minz) / length);
      nc = cubes[depth][xindex][yindex][zindex];
      if(nc == NULL) {
 ...
 *  in "cube."
 */
int cntDwnwdChg(cube *cp, int depth)            /* depth: number of lowest level */
{
  int i;
  int cnt;
  cnt = 0;
