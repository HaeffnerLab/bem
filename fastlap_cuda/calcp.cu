...
#define SEVEN3 2.333333333333
#define ONE6   0.166666666667
#define ONE3   0.333333333333
...
static void eulerAngles(double X[3], double Y[3], double Z[3], double *pcosa, double *psina, double *pcosb, double *psignb, double *pcosg, double *psing);
...
void initcalcp(snglrty *sing_list, int order, double* pAreas)
{
    ...
    fprintf(stdout, "FLW-initcalcp: Panel skewed beyond the PLANAR_TOL tolerance.\n");
            ...
            // this is necessary for calculating the gradient of the potential in calcp()
            if (pq->shape == 3) {
                for (i=0; i<3; i++)
...
/*
// 100330 xxxxx Start of temporary stuff for Mike (output triangles' areas) xxxxx
    {
        int iSingularity = 0;
        FILE *fAreaOut;
        if ((fAreaOut = fopen("TriangleAreas.dat", "w")) == NULL) {
            fprintf(stderr, "\ncalcp : Can't open file TriangleAreas.dat for data output.\n");
            exit(0);
        }
        pq = sing_list;
        while (pq != NULL) {
            iSingularity++;
            fprintf(fAreaOut, "%8d %e\n", iSingularity, pq->area);
            
            // Iterate for the next singularity.
            pq = pq->next;
        }
        if (fAreaOut!=NULL) fclose(fAreaOut);
    }
// 100330 xxxxx End of temporary stuff for Mike (output triangles' areas) xxxxx
*/
...
// Determines centroid of a panel (defined as the point which makes the first moments vanish.
// Calculation begins by projection into the coordinate system defined by the panel normal
// as the z-axis and edge02 as the x-axis.
...
// Used to make sure that arrays for computeMoments and Calcspherical are recomputed
...
/*
    ...
    may need quite a bit of the code found in calcSphericalPanel because they
...
void calcSphericalPoint(snglrty *pp, int order)
{
    ...
    /* Since pp->order exists, does that mean that individual sings can have
    ...
    /* Here you need to set up for whatever singularities you need.*/
    ...
    if (pp->rhsType == POINT_SOURCE) {
        ...
        fprintf(stdout,"FLE-calcpsph: unknown point singularity type: %d\n",pp->lhsType);
    }
...
/* fill in the point source definition here.*/
    multiM[iindex(0,0)] = 1.0;
}
...
void calcSphericalPanel(snglrty *pp, double **I, int order)
{
    ...
    else if(pp->rhsType == NO_SOURCE) {
        pp->multipoleR = multiM;                // Yves : this will not really be used anyway
    }
    else {
        fprintf(stdout,"FLE-calcpsph: unknown singularity type: %d\n",pp->lhsType);
    }
    ...
    else if(pp->lhsType == NO_SOURCE) {
        pp->multipoleL = multiM;                // Yves : this will not really be used anyway
    }
    else {
        fprintf(stdout,"FLE-calcpsph: unknown singularity type: %d\n",pp->lhsType);
    }
    ...
    for(n = 0; n <= order; n++) {
        multiM[iindex(n,0)] = Mrmn[0][n];
        /*flip dipole sign here to agree with the panel normal convention.*/
        multiD[iindex(n,0)] = -Nrmn[0][n];
        for(m = 1; m <= n; m++) {
            multiM[iindex(n,m)] = Mrmn[m][n];
            multiM[sindex(n,m,rterms)] = Mrtmn[m][n];
            /*flip dipole sign here to agree with the panel normal convention.*/
            multiD[iindex(n,m)] = -Nrmn[m][n];
...
/* jt: variables for gradient evaluations */
    ...
    double fsx=0., fsy=0., fsz=0., fdx=0., fdy=0., fdz=0.;
    double xri[4], yri[4], fln, fac, fh1, fh2, u1, u2, rr;
    double nDrvSrc=0., nDrvDip=0.;
 ...
 */
    ...
    else if( sing->lhsType == NO_SOURCE ) {
        *pfd = 0;
    }
    else {
        fprintf(stderr, "FLE-calcp: unknown singularity type: %d\n",sing->lhsType);
    }
    }
    ...
    else if( sing->rhsType == NO_SOURCE ) {
        return(0);
    }
    else {
        fprintf(stderr,"FLE-calcp: unknown singularity type: %d\n",sing->rhsType);
