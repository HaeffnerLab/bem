...
// Any #define precompiler statements must be on a single line,
// hence the heavy use of backslashes (continuation character)

#ifndef _mulglobal_h_included_
#define _mulglobal_h_included_
...
extern size_t memQ2M;
extern size_t memQ2L;
extern size_t memQ2P;
extern size_t memL2L;
extern size_t memM2M;
extern size_t memM2L;
extern size_t memM2P;
extern size_t memL2P;
extern size_t memMSC;
...
/* # ***** awked out for release */                                           \
  (void)fprintf(stderr, " Q2M  matrix memory allocated: %7.zd kilobytes\n",    \
                memQ2M/1000);                                                 \
  memcount = memQ2M;                                                          \
  (void)fprintf(stderr, " Q2L  matrix memory allocated: %7.zd kilobytes\n",    \
                memQ2L/1000);                                                 \
  memcount += memQ2L;                                                         \
  (void)fprintf(stderr, " Q2P  matrix memory allocated: %7.zd kilobytes\n",    \
                memQ2P/1000);                                                 \
  memcount += memQ2P;                                                         \
  (void)fprintf(stderr, " L2L  matrix memory allocated: %7.zd kilobytes\n",    \
                memL2L/1000);                                                 \
  memcount += memL2L;                                                         \
  (void)fprintf(stderr, " M2M  matrix memory allocated: %7.zd kilobytes\n",    \
                memM2M/1000);                                                 \
  memcount += memM2M;                                                         \
  (void)fprintf(stderr, " M2L  matrix memory allocated: %7.zd kilobytes\n",    \
                memM2L/1000);                                                 \
  memcount += memM2L;                                                         \
  (void)fprintf(stderr, " M2P  matrix memory allocated: %7.zd kilobytes\n",    \
                memM2P/1000);                                                 \
  memcount += memM2P;                                                         \
  (void)fprintf(stderr, " L2P  matrix memory allocated: %7.zd kilobytes\n",    \
                memL2P/1000);                                                 \
  memcount += memL2P;                                                         \
  (void)fprintf(stderr, " Miscellaneous mem. allocated:%7.zd kilobytes\n",     \
                memMSC/1000);                                                 \
  memcount += memMSC;                                                         \
  (void)fprintf(stderr, " Total memory (check w/above):%7.zd kilobytes\n\n",   \
                memcount/1000);                                               \
/* # ***** awked out for release */                                           \
}

#define CALLOC(PNTR, NNUM, TYPE, FLAG, MTYP)                                  \
{                                                                             \
    size_t NUM = (size_t) NNUM;                                               \
     if((NUM)*sizeof(TYPE)==0) {(PNTR) = (TYPE*) NULL;}                       \
     else if(((PNTR)=(TYPE*)CALCORE(NUM, TYPE))==NULL) {                      \
       (void)fprintf(stderr,                                                  \
         "FLE-%s: out of memory at line %d\n",                                \
               __FILE__, __LINE__);                                           \
       (void)fprintf(stderr, " (NULL pointer on %zd byte request)\n",         \
                     (NUM)*sizeof(TYPE));                                     \
       DUMPALLOCSIZ;                                                          \
       DUMPRSS;                                                               \
       (void)fflush(stderr);                                                  \
...
#define MALLOC(PNTR, NNUM, TYPE, FLAG, MTYP)                                   \
{                                                                             \
    size_t NUM = (size_t) NNUM;                                               \
     if((NUM)*sizeof(TYPE)==0) {(PNTR) = (TYPE*) NULL;}                       \
...
#define NO_SOURCE 0             /* xxx Yves xxx Indicates snglrty bears no source (for either LHS or RHS) */
...
#define PRECOND NONE            /* Preconditioner, NONE, OL. */
